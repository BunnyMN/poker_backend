...
export interface FiveCardClassification {
  kind: FiveCardKind;
  tiebreak: {
    // For STRAIGHT/STRAIGHT_FLUSH: highest card rank in straight
    highestRank?: Rank;
    // For FLUSH: sorted cards (rank then suit desc)
    sortedCards?: Card[];
    // For FULL_HOUSE: triplet rank, pair rank, highest suit in triplet
    tripletRank?: Rank;
    pairRank?: Rank;
    tripletHighestSuit?: string;
    // For FOUR: four rank, kicker rank, highest suit in four
    fourRank?: Rank;
    kickerRank?: Rank;
    fourHighestSuit?: string;
    // For all: highest suit among all 5 cards (for final tie-break)
    highestSuit?: string;
  };
}
 ...
 */
function isStraight(cards: Card[]): { isStraight: boolean; highestRank?: Rank } {
  if (cards.length !== 5) {
...
function isFullHouse(cards: Card[]): {
  ...
  let tripletRank: Rank | undefined;
  let pairRank: Rank | undefined;
  for (const [rank, count] of Object.entries(rankCounts) as [Rank, number][]) {
    if (count === 3) {
      tripletRank = rank;
    } else if (count === 2) {
      pairRank = rank;
 ...
 */
function getHighestSuit(cards: Card[]): string {
 ...
 */
export function classifyFiveCardHand(
  cards: Card[]
): FiveCardClassification | null {
  if (cards.length !== 5) {
    return null;
  }
  ...
  if (fullHouseResult.isFullHouse) {
    const tripletCards = cards.filter(
      (c) => c.rank === fullHouseResult.tripletRank
    );
    return {
      kind: 'FULL_HOUSE',
      tiebreak: {
...
export function compareFiveCardHands(
  a: FiveCardClassification,
  b: FiveCardClassification
): number {
  // Hand rank order: Straight < Flush < Full House < Four < Straight Flush
  const HAND_RANK_ORDER: Record<FiveCardKind, number> = {
    STRAIGHT: 1,
    FLUSH: 2,
    FULL_HOUSE: 3,
    FOUR: 4,
  ...
  if (a.kind === 'STRAIGHT' || a.kind === 'STRAIGHT_FLUSH') {
    ...
    const aSuit = SUIT_ORDER[tiebreakA.highestSuit as keyof typeof SUIT_ORDER];
    const bSuit = SUIT_ORDER[tiebreakB.highestSuit as keyof typeof SUIT_ORDER];
    if (aSuit !== bSuit) {
  ...
  if (a.kind === 'FULL_HOUSE') {
    ...
    const aSuit =
      SUIT_ORDER[tiebreakA.tripletHighestSuit as keyof typeof SUIT_ORDER];
    const bSuit =
      SUIT_ORDER[tiebreakB.tripletHighestSuit as keyof typeof SUIT_ORDER];
    if (aSuit !== bSuit) {
      return aSuit < bSuit ? -1 : 1;
    }
  ...
  if (a.kind === 'FOUR') {
    ...
    const aSuit =
      SUIT_ORDER[tiebreakA.fourHighestSuit as keyof typeof SUIT_ORDER];
    const bSuit =
      SUIT_ORDER[tiebreakB.fourHighestSuit as keyof typeof SUIT_ORDER];
    if (aSuit !== bSuit) {
      return aSuit < bSuit ? -1 : 1;
    }
