 ...
 */
export function getWeakestSingleOwner(
  hands: Record<string, Card[]>,
  seatedPlayerIds: string[]
): { playerId: string; card: Card } {
  let weakestOwner: { playerId: string; card: Card } | null = null;
...
export function determineStarter(
  params: DetermineStarterParams
): DetermineStarterResult {
  ...
  if (tableUnchanged) {
    if (!previousWinnerPlayerId) {
      throw new Error(
        'tableUnchanged is true but previousWinnerPlayerId is not provided'
      );
    }
    return {
