...
async function dealAndSendCards(roomId) {
    ...
    if (!seatedPlayerIds || seatedPlayerIds.length === 0) {
        ...
        const allPlayers = Array.from(room.players.keys()).filter((id) => !room.eliminated.has(id));
    ...
    const tableUnchanged = previousSeatedPlayerIds.length === seatedPlayerIds.length &&
 ...
 */
function getNextActivePlayer(currentPlayerId, seatedPlayerIds, hands) {
 ...
 */
function sendPersonalState(roomId, playerId, ws) {
    const room = getRoom(roomId);
    if (!room || room.phase !== 'playing') {
        return;
    }
 ...
 */
function sendSyncState(roomId, playerId, ws) {
    const room = getRoom(roomId);
    if (!room) {
        return;
    }
    // Build totalScores object
    const totalScoresObj = {};
 ...
 */
function broadcastRoomOverview(roomId) {
    const room = getRoom(roomId);
    if (!room) {
        return;
    }
 ...
 */
function checkAndHandleTrickEnd(roomId) {
    const room = getRoom(roomId);
    if (!room || room.phase !== 'playing') {
        return;
    }
    ...
    if (passedActiveCount === activePlayerCount - 1) {
        app.log.info(`Trick ended: roomId=${roomId}, winner=${lastPlayPlayerId}, passedActiveCount=${passedActiveCount}, activePlayerCount=${activePlayerCount}`);
 ...
 */
function calculateScores(room, winnerPlayerId) {
 ...
 */
function applyElimination(room, winnerPlayerId) {
    if (!room) {
        return [];
    }
...
function getRemainingPlayers(room) {
    ...
    return Array.from(room.players.keys()).filter((playerId) => !room.eliminated.has(playerId));
}
/**
 ...
 */
function findFollowOutPlayer(room, winnerPlayerId) {
    if (!room || !room.hands || !room.seatedPlayerIds) {
        return null;
    }
 ...
 */
function performRotation(room, winnerPlayerId) {
    ...
    let currentSeated = room.seatedPlayerIds.filter((id) => !room.eliminated.has(id));
    if (isDoubleOut) {
        // DOUBLE_OUT mode
        const followOutPlayerId = findFollowOutPlayer(room, winnerPlayerId);
        // Remove winner and follow-out from seated
        currentSeated = currentSeated.filter((id) => id !== winnerPlayerId && id !== followOutPlayerId);
 ...
 */
async function checkAndHandleRoundEnd(roomId, winnerPlayerId) {
    const room = getRoom(roomId);
    if (!room || room.phase !== 'playing') {
        return;
    }
    ...
    app.log.info(`Rotation: roomId=${roomId}, newSeated=${newSeatedPlayerIds.join(',')}, queue=${room.queuePlayerIds.join(',')}`);
 ...
 * Handle PLAY message
 */
function handlePlay(roomId, playerId, cards, ws) {
    const room = getRoom(roomId);
    if (!room) {
        const error = {
            type: 'ACTION_ERROR',
    ...
    if (playerId !== room.currentTurnPlayerId) {
        ...
        ws.send(JSON.stringify(error));
        app.log.warn(`PLAY validation failed: roomId=${roomId}, playerId=${playerId}, currentTurn=${room.currentTurnPlayerId}`);
        return;
    }
    ...
    if (cards.length !== 1 && cards.length !== 2 && cards.length !== 3 && cards.length !== 5) {
        ...
        ws.send(JSON.stringify(error));
        app.log.warn(`PLAY validation failed: roomId=${roomId}, playerId=${playerId}, cardCount=${cards.length}`);
        return;
    }
    ...
    for (const card of cards) {
        ...
        if (index === -1) {
            ...
            ws.send(JSON.stringify(error));
            app.log.warn(`PLAY validation failed: roomId=${roomId}, playerId=${playerId}, card=${card.rank}${card.suit} not in hand`);
            return;
        }
    ...
    if (uniqueIndices.size !== cardIndices.length) {
        ...
        ws.send(JSON.stringify(error));
        app.log.warn(`PLAY validation failed: roomId=${roomId}, playerId=${playerId}, duplicate cards`);
        return;
    }
    ...
    if (cards.length === 1) {
        ...
        if (!isPair(cards)) {
            ...
            ws.send(JSON.stringify(error));
            app.log.warn(`PLAY validation failed: roomId=${roomId}, playerId=${playerId}, invalid PAIR`);
            return;
        }
        playKind = 'PAIR';
    }
        ...
        if (!isSet(cards)) {
            ...
            ws.send(JSON.stringify(error));
            app.log.warn(`PLAY validation failed: roomId=${roomId}, playerId=${playerId}, invalid SET`);
            return;
        }
        playKind = 'SET';
    }
        ...
        if (!classification) {
            ...
            ws.send(JSON.stringify(error));
            app.log.warn(`PLAY validation failed: roomId=${roomId}, playerId=${playerId}, invalid 5-card hand`);
            return;
        }
        playKind = 'FIVE';
    ...
    if (room.lastPlay) {
        ...
        if (cards.length !== room.lastPlay.cards.length) {
            ...
            ws.send(JSON.stringify(error));
            app.log.warn(`PLAY validation failed: roomId=${roomId}, playerId=${playerId}, cardCount mismatch: ${cards.length} vs ${room.lastPlay.cards.length}`);
            return;
        }
        ...
        if (comparison <= 0) {
            ...
            ws.send(JSON.stringify(error));
            app.log.warn(`PLAY validation failed: roomId=${roomId}, playerId=${playerId}, play too weak`);
            return;
        }
    }
    ...
    app.log.info(`PLAY: roomId=${roomId}, playerId=${playerId}, kind=${playKind}${fiveKindStr}, cards=[${cardsStr}]`);
 ...
 * Handle PASS message
 */
function handlePass(roomId, playerId, ws) {
    const room = getRoom(roomId);
    if (!room) {
        const error = {
            type: 'ACTION_ERROR',
    ...
    if (playerId !== room.currentTurnPlayerId) {
        ...
        ws.send(JSON.stringify(error));
        app.log.warn(`PASS validation failed: roomId=${roomId}, playerId=${playerId}, currentTurn=${room.currentTurnPlayerId}`);
        return;
    }
    ...
    if (!room.lastPlay) {
        ...
        ws.send(JSON.stringify(error));
        app.log.warn(`PASS validation failed: roomId=${roomId}, playerId=${playerId}, starter cannot pass (lastPlay is null)`);
        return;
    }
    ...
    ws.on('message', async (data) => {
        try {
            ...
            if (message.type === 'PLAY') {
                ...
                handlePlay(message.roomId, authenticatedData.playerId, message.cards, ws);
                return;
            }
            // Handle PASS
            ...
            if (message.type === 'SET_RULES') {
                ...
                app.log.info(`SET_RULES: roomId=${message.roomId}, owner=${authenticatedData.playerId}, scoreLimit=${message.scoreLimit}`);
