...
const getRankOrder = (rank) => RANK_ORDER[rank];
const getSuitOrder = (suit) => SUIT_ORDER[suit];
/**
 ...
 */
export function comparePair(aCards, bCards) {
 ...
 * Returns: -1 if a < b, +1 if a > b, 0 if equal
 */
export function compareSet(aCards, bCards) {
    ...
    const aMaxSuit = Math.max(...aCards.map((c) => getSuitOrder(c.suit)));
    const bMaxSuit = Math.max(...bCards.map((c) => getSuitOrder(c.suit)));
